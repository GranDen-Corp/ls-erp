...
export async function generateOrderNumber(): Promise<string> {
  try {
    ...
    if (monthlyOrders && monthlyOrders.length > 0) {
      ...
      // 檢查是否符合 L-YYMMXXXXX 格式
      const match = latestOrderId.match(/^L-(\d{4})(\d{5})$/)
      if (match && match[1] === yearMonth) {
        ...
        console.log("訂單編號格式不符合 L-YYMMXXXXX，使用預設序號 1")
      }
    } else {
...
export function validateOrderNumber(orderNumber: string): boolean {
  // 檢查是否為 L-YYMMXXXXX 格式 (L-加上9位數字)
  const regex = /^L-\d{4}\d{5}$/
