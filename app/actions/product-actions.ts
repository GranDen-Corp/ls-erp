...
export async function saveProduct(formData: any) {
  try {
    ...
    const productData = {
      ...
      last_price: formData.lastPrice ? parseFloat(formData.lastPrice) : null,
      currency: formData.currency || 'USD', // 如果沒有提供貨幣，預設使用 USD
      ...
      additional_costs: formData.additionalCosts ? parseFloat(formData.additionalCosts) : null,
      ...
      mold_cost: formData.moldCost ? parseFloat(formData.moldCost) : null,
      refundable_mold_quantity: formData.refundableMoldQuantity ? parseInt(formData.refundableMoldQuantity) : null,
